...
surveyCakeRaw.trim().split('\n').forEach(line => {
  ...
  if (!emp) {
    console.log('Unknown empId in SurveyCake:', empId);
    return;
  }
  ...
  if (emp.group === 'fat') {
    ...
    emp.bodyResult = diff; // can be negative if gained fat
    emp.isGoalMet = diff >= targetStandard;
  } else {
    // 增肌kg = postMuscle - preMuscle
...
// 1. Calculate InBody Rankings & Points for Fat Group
...
// 2. Calculate InBody Rankings & Points for Muscle Group
const muscleList = Array.from(empMap.values()).filter(e => e.group === 'muscle');
muscleList.sort((a, b) => b.bodyResult - a.bodyResult);

// Muscle participants count check (18 participants >= 15, so Top 3)
const muscleTopCap = muscleList.length < 15 ? 2 : 3;
...
// 3. Individual Competition Ranking (Sort by totalPts DESC, then bodyResult DESC)
...
// Determine Fat Individual Winners (Top 3 eligible: isGoalMet && totalPts >= 45)
...
// Determine Muscle Individual Winners (Top 3 eligible: isGoalMet && totalPts >= 45)
...
console.log('================ FAT INDIVIDUAL RANKINGS ================');
fatIndividual.forEach((e, idx) => {
  console.log(
    `#${idx + 1} [${e.empId}] ${e.name} (${e.nickname}) | 任務分:${e.taskPts} + 體態分(名次:${e.rankPts}/級距:${e.inbodyPts}) = 總分:${e.totalPts} | 前測:${e.preFat}% 後測:${e.postFat}% => 減脂:${e.bodyResult}% (標:${e.targetStandard}%) 達標:${e.isGoalMet ? 'O' : 'X'} | 達標獎:${e.achievementAward ? '$2000' : '-'} | 競賽獎:${e.individualAward || '-'}`
  );
});


console.log('\n================ MUSCLE INDIVIDUAL RANKINGS ================');
muscleIndividual.forEach((e, idx) => {
  console.log(
    `#${idx + 1} [${e.empId}] ${e.name} (${e.nickname}) | 任務分:${e.taskPts} + 體態分(名次:${e.rankPts}/級距:${e.inbodyPts}) = 總分:${e.totalPts} | 前測:${e.preMuscle}kg 後測:${e.postMuscle}kg => 增肌:${e.bodyResult}kg (標:${e.targetStandard}kg) 達標:${e.isGoalMet ? 'O' : 'X'} | 達標獎:${e.achievementAward ? '$2000' : '-'} | 競賽獎:${e.individualAward || '-'}`
  );
});

console.log('\n================ FAT TEAMS ================');
fatTeams.forEach((t, idx) => {
  console.log(
    `#${idx + 1} 第${t.teamNum}組 (${t.members.map(m => m.name).join(', ')}) | 平均總分:${t.avgTotalPts} | 平均減脂:${t.avgBodyResult}% (標:${t.avgTargetStandard}%) | 達標:${t.isAvgGoalMet} | 全員>=45分:${t.isAllMembersMinPtsMet} | 獲獎:${t.award || '無'}`
  );
});

console.log('\n================ MUSCLE TEAMS ================');
muscleTeams.forEach((t, idx) => {
  console.log(
    `#${idx + 1} 第${t.teamNum}組 (${t.members.map(m => m.name).join(', ')}) | 平均總分:${t.avgTotalPts} | 平均增肌:${t.avgBodyResult}kg (標:${t.avgTargetStandard}kg) | 達標:${t.isAvgGoalMet} | 全員>=45分:${t.isAllMembersMinPtsMet} | 獲獎:${t.award || '無'}`
  );
});
